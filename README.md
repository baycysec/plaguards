...
|:-:|:------------|:----|
|1. | Powershell Deobfuscation| Plaguards introduces deobfuscation features such as:<br>a. Concat strings value and variable.<br>b. Concat strings value that has the same variable.<br>c. Decode base64 -> has 2 patterns: "==" at the end of the string and use "base64fromstring" function.<br>d. Evaluate Backtick.<br>e. Seperate Code into Several Lines -> ";" (semicolon) as the separator.<br>f. Flexible variable value change.<br>g. Arithmatic operation inside char (plus (+), minus (-), multiply (*), divide (/), XOR (-bxor)).<br>h. Evaluate replace.<br>i. Evaluate split.<br>j. Evaluate join.<br>k. Recognize and evaluate whitespace.<br>l. Process ASCII.getstring function.<br>m. Obtain IOC Parameter Input (Domain and IP) |
